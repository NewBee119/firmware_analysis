...
if ($generate_start == 1)
{
	echo "echo \"It is the DHCPPLUS mod start!!\" > /dev/console\n";
	anchor("/wan/rg/inf:1/dhcpplus");
	$hostname	= query("/sys/hostname");
	$clonemac	= query("clonemac");
	$autodns	= query("autodns");
	$mtu		= query("mtu");
	$dhcpplus_username		= query("dhcpplususer");
	$dhcpplus_passwd		= query("dhcppluspassword");
	...
	//echo "sleep 1\n"; // peter
	echo "dhcpplus -U ".$dhcpplus_username." -P ".$dhcpplus_passwd." &\n";	
	echo "udhcpc -i ".$wanif." -p ".$dhcpc_pid.$HOST." -m -s /var/run/udhcpc.sh -D 2 -R 5 -S 300 &\n";
...
{
	echo "echo \"Stop DHCPPLUS client on WAN(".$wanif.") ...\" > /dev/console\n";
	echo "if [ -f ".$dhcpplus_pid." ]; then\n";
	echo "	PID=`cat ".$dhcpplus_pid."`\n";
	echo "	if [ $PID != 0 ]; then\n";
	echo "		kill $PID > /dev/console 2>&1\n";
	echo "	fi\n";
	echo "	rm -f ".$dhcpplus_pid."\n";
	echo "fi\n";
